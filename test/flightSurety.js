...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it(`(multiparty) has correct first airline data`, async function () {
        ...
        // await config.flightSuretyApp.registerAirline(newAirline1.address, newAirline1.name, newAirline1.code, { from: config.owner });
    });
*/
    ...
    it(`(multiparty) can block access to functions using requireIsOperational when operating status is false`, async function () {
        ...
        await config.flightSuretyApp.setOperational(false, { from: config.owner });
        let operational = await config.flightSuretyApp.isOperational.call({ from: config.owner });
        let reverted = false;

        try {
            await config.flightSuretyApp.setTestingMode(true, { from: config.owner });
        }
        catch (e) {
            reverted = true;
        }
        ...
        await config.flightSuretyApp.setOperational(true, { from: config.owner });
